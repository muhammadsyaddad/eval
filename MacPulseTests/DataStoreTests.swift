...
class DatabaseTestCase: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        let tmpURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("macpulse_test_\(UUID().uuidString).db")
...
final class FullTextSearchTests: DatabaseTestCase {
    ...
    func testSearchDailySummaries() throws {
        try dataStore.upsertDailySummary(makeSummary(
            aiSummary: "Productive development day focused on MacPulse OCR pipeline"
        ))
        try dataStore.upsertDailySummary(makeSummary(
        ...
        let results = try dataStore.searchDailySummaries(query: "MacPulse", limit: 10)
        XCTAssertEqual(results.count, 1)
